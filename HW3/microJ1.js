...
class WithValue { 		//!! id ve = ve number ın hepsini alıp a=6 gibi kullanmamızı sağlayan kod tostring ile sol "harf" = "sayı"
   constructor(v, e) { this.left = v; this.right = e; }
...
function identList() {
    ...
    while (tok == COMMA)  {
      match(COMMA); L.push(withValue()); //match ediyoruz virgülü alıp sonra withvalue gelmesini bekliyoruz value kesin gelmeli virgülden sonra
    }
    return L;
}
...
function withValue() {
   ...
   if (tok == ASSIGN) {  //eğer assign yani eşittir gelirse id den sonra -> eşittirden sonra number bekliyor oda expression!
      match(ASSIGN);
      let e = expression(); //expression -> number
      return new WithValue(L, e); //daha sonra bu a=4 gibi olan yapının tamamını kullanabilmek için withvalue ye gönderiyoruz kodu!!
   }
   return L;
}
